...
Then(/^Inventory page should (.*)\s?list (.*)$/, async function (negativeMatch, noOfProducts){
    try {
        ...
        throw err //failing
        logger.error(err.message) //report & move on
    }
})
...
//    console.log(`>>Price with number: ${priceNumArr}`)
 ...
 */
 Then(/^Verify if all users exist in customer list$/, async function () {
    ...
    try {
        /**1. Navigate/select Customer options from left menu*/
        // @ts-ignore
        await browser.url(`${browser.config.nopCommerceBaseURL}/Admin/Customer/List`)
